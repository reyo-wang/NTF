...
def parafac(tensor, rank, n_iter_max=100, init='svd', tol=1e-7,
    ...
    for iteration in tqdm(range(n_iter_max),desc= 'parafac:'):
        for mode in range(T.ndim(tensor)):
...
#### based on tensorly re-write a weighted non negative parafacterization
def weighted_non_negative_parafac(tensor, rank, w_tensor, n_iter_max=100, init='svd', tol=10e-7,
    ...
    for iteration in tqdm(range(n_iter_max),desc= 'Non-negative parafac:'):
        for mode in range(T.ndim(tensor)):
            ...
            numerator = T.dot(T.dot(unfold(tensor, mode),unfold(w_tensor, mode)), khatri_rao(nn_factors, skip_matrix=mode))   #   dot w_tensor
            numerator = T.clip(numerator, a_min=epsilon, a_max=None)
            #denominator = T.dot(nn_factors[mode], accum)
            denominator = T.clip(denominator, a_min=epsilon, a_max=None)
            nn_factors[mode] = nn_factors[mode]* numerator / denominator
